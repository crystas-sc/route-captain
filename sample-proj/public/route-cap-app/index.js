	...
	class RouteCaptainChat extends HTMLElement {
		...
		_render() {
			...
			panel.innerHTML = `
				...
				<div class="px-4 py-2">
					<select class="rc-tour-select w-full text-xs rounded-md px-2 py-1 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100">
							<option value="">Start a Tour</option>
							${tours.map(tour => `<option value="${tour.name}">${tour.label}</option>`).join('')}
						</select>
				</div>
		...
		_attachEvents() {
			...
			this._onTourSelect = this._onTourSelect.bind(this);
			...
			const tourSelect = this._panel.querySelector('.rc-tour-select');
			...
			if (tourSelect) tourSelect.addEventListener('change', this._onTourSelect);
		...
		_onTourSelect(e) {
			...
			this._closePanel(); // Close chat panel when tour starts
